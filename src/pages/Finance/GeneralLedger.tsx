...
const GeneralLedger: React.FC = () => {
  ...
  const loadData = () => {
    ...
    if (entries.length === 0) {
      ...
      sampleEntries.forEach(entry => upsertEntity(ENTRIES_STORAGE_KEY, entry as any));
    ...
    if (chartAccounts.length === 0) {
      ...
      sampleAccounts.forEach(account => upsertEntity(ACCOUNTS_STORAGE_KEY, account as any));
  ...
  const onSubmitEntry = (data: z.infer<typeof journalEntrySchema>) => {
    try {
      ...
      upsertEntity(ENTRIES_STORAGE_KEY, newEntry as any);
  ...
  const onSubmitAccount = (data: z.infer<typeof accountSchema>) => {
    try {
      ...
      upsertEntity(ACCOUNTS_STORAGE_KEY, newAccount as any);
  ...
  const postEntry = (entry: JournalEntry) => {
    try {
      ...
      upsertEntity(ENTRIES_STORAGE_KEY, updatedEntry as any);
  ...
  const reverseEntry = (entry: JournalEntry) => {
    ...
    try {
      ...
      upsertEntity(ENTRIES_STORAGE_KEY, reversedEntry as any);
  ...
  const entryActions: TableAction[] = [
    {
      label: 'View',
